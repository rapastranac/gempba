...
namespace gempba {
    ...
    class MPISemiCentralizedScheduler final : public Scheduler {
        ...
        std::vector<int> _elapsed_time_per_process{};
        std::vector<int> _idle_time_per_process{};
    ...
    public:
        ...
        void synchronize_statistics() override {
            ...
            int array[_world_size];

            if (_world_rank == 0) {
                _received_tasks_per_process.resize(_world_size);
                _sent_tasks_per_process.resize(_world_size);
                _elapsed_time_per_process.resize(_world_size);
                _idle_time_per_process.resize(_world_size);

                MPI_Gather(NULL, 1, MPI_INT, array, 1, MPI_INT, CENTER_NODE, _world_communicator);
            } else {
        ...
        [[nodiscard]] double get_elapsed_time(int rank) const override {
