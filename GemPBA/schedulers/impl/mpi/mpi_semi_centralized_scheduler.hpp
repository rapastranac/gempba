...
namespace gempba {
    ...
    public:
        ...
        void runNode(BranchHandler &branchHandler) override {
            ...
            send_final_solution_to_center(branchHandler);
            _idle_time = branchHandler.idle_time();
            synchronize_statistics();
        }
        ...
        void synchronize_statistics() override {
            ...
            if (_world_rank == CENTER_NODE) {
                ...
                MPI_Gather(MPI_IN_PLACE, 1, MPI_INT, _received_tasks_per_process.data(), 1, MPI_INT, CENTER_NODE, _world_communicator);
                MPI_Gather(MPI_IN_PLACE, 1, MPI_INT, _sent_tasks_per_process.data(), 1, MPI_INT, CENTER_NODE, _world_communicator);
                MPI_Gather(MPI_IN_PLACE, 1, MPI_DOUBLE, _idle_time_per_process.data(), 1, MPI_DOUBLE, CENTER_NODE, _world_communicator);
            } else {
                MPI_Gather(&_received_tasks, 1, MPI_INT, NULL, 1, MPI_INT, CENTER_NODE, _world_communicator);
                MPI_Gather(&_sent_tasks, 1, MPI_INT, NULL, 1, MPI_INT, CENTER_NODE, _world_communicator);
                MPI_Gather(&_idle_time, 1, MPI_DOUBLE, NULL, 1, MPI_DOUBLE, CENTER_NODE, _world_communicator);
            }
            barrier();
        ...
        void barrier() override {
            if (_world_communicator == MPI_COMM_NULL) {
                utils::log_and_throw("Attempting to call barrier on MPI_COMM_NULL");
            }
            MPI_Barrier(_world_communicator);
        }
        ...
        [[nodiscard]] double get_elapsed_time() const override {
            ...
            return utils::difftime(_start_time, _end_time) - static_cast<double>(TIMEOUT_TIME);
        }
    ...
    public:
        ...
        void push(std::string &&message, int function_id) override {
            if (message.empty()) {
                auto msg = fmt::format("rank {}, attempted to send an empty buffer to rank {} \n", _world_rank, _next_process);
            ...
            utils::shift_left(_next_processes.data(), static_cast<int>(_next_processes.size()));
            ...
            if (!_tasks_queue.empty()) {
                auto msg = "ERROR: _tasks_queue is not empty !!!!\n";
                spdlog::error(msg);
                throw std::runtime_error(msg);
            }
        ...
        */
        ...
        [[nodiscard]]bool receiveOrTimeout(int buffer, int &ready, double wTime0, MPI_Status &status, MPI_Request &request) {
            ...
            while (true) {
                ...
                while (!ready && (TIMEOUT_TIME > utils::difftime(wTime0, wTime1))) {
                    MPI_Test(&request, &ready, &status);
                    cycles++;
                }
        ...
        void process_available(const MPI_Status &status) {
            ...
            for (int i_rank = 0; i_rank < _world_size; i_rank++) {
                ...
                if (rank <= 0) continue;
                if (_process_state[rank] != STATE_RUNNING) continue; // finds the first running node
        ...
        void maybe_broadcast_global_reference_value(int new_global_reference_value, MPI_Status &status) {
            ...
            switch (_lookup_strategy) {
                ...
                default:
                    throw std::runtime_error("Unrecognised lookup strategy");
            }
    ...
    */
        ...
        void process_message(BranchHandler &branchHandler) {
            MPI_Status status;
            ...
            std::optional<std::shared_future<std::string>> returned_value_opt = branchHandler.processIncomingBuffer(message, count, function_id);
         ...
         * @param branchHandler
         */
        void task_funneling(BranchHandler &branchHandler) {
            ...
            while (true) {
...

                {
                    ...
                    maybe_update_reference_value(branchHandler);
        ...
        void send_task(const std::shared_ptr<std::pair<std::string, int>> &task) {
            ...
            if (_destination_rank > 0) {
                if (_destination_rank == _world_rank) {
                    auto msg = "rank " + std::to_string(_world_rank) + " attempting to send to itself !!!\n";
                    throw std::runtime_error(msg);
                }
                ...
                auto msg = "rank " + std::to_string(_world_rank) + ", could not send task to rank " + std::to_string(_destination_rank) + "\n";
                throw std::runtime_error(msg);
            }
        }
        ...
        int probe_next_process() {
            ...
            if (isMessageReceived) {
                ...
                //TODO .. should throw if is count is not in range
        ...
        void maybe_send_result_back_to_non_void_source_rank() {
            ...
            throw std::runtime_error("Non-void result not supported yet");
        ...
        void maybe_update_reference_value(BranchHandler &branchHandler) {
            const int _local_reference_value = branchHandler.refValue();

            if (should_update_local(_lookup_strategy, _global_reference_value, _local_reference_value)) {
                branchHandler.updateRefValue(_global_reference_value);
        ...
        void send_final_solution_to_center(BranchHandler &branchHandler) {
            std::optional<std::pair<int, std::string>> best_solution_opt = branchHandler.getSerializedBestSolution();
