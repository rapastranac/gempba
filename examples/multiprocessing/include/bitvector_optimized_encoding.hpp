#ifndef GEMPBA_MULTIPROCESSING_BITVECTOR_OPTIMIZED_ENCODING_HPP
#define GEMPBA_MULTIPROCESSING_BITVECTOR_OPTIMIZED_ENCODING_HPP
...
class multiprocessing_bitvector_optimized_encoding final {
...
public:
    ...
    explicit multiprocessing_bitvector_optimized_encoding(gempba::node_manager &p_node_manager, gempba::load_balancer *p_load_balancer) :
        m_node_manager(p_node_manager), m_load_balancer(p_load_balancer) {

        this->m_function = std::bind(&multiprocessing_bitvector_optimized_encoding::mvcbitset, this, _1, _2, _3, _4, _5);
        this->m_args_deserializer = create_deserializer();
        this->m_args_serializer = make_serializer();
    }

    ~multiprocessing_bitvector_optimized_encoding() = default;
    ...
    void some_rules_applies(G_BITSET &p_bits_in_graph, int &p_cursol_size, int &p_maxdeg, int &p_maxdeg_v, int &p_nb_edges_double_counted) {
...
private:
    void terminate_condition_bits(int p_solution_size, int p_depth) const {
        if (p_solution_size == 0) {
            spdlog::debug("rank {}, exiting due to p_solution_size: {}, p_depth: {}", m_node_manager.rank_me(), p_solution_size, p_depth);
            return;
        }
        ...
        if (p_solution_size < m_node_manager.get_score().get<int>()) {
            ...
            const bool v_updated = m_node_manager.try_update_result(p_solution_size, gempba::score::make(p_solution_size), v_serializer);
...
#endif // GEMPBA_MULTIPROCESSING_BITVECTOR_OPTIMIZED_ENCODING_HPP
