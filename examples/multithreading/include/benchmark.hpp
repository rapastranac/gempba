#ifndef GEMPBA_MULTITHREADING_BENCHMARK_HPP
#define GEMPBA_MULTITHREADING_BENCHMARK_HPP
...
class multithreading_benchmark {
...
public:
    multithreading_benchmark(gempba::node_manager &p_node_manager, gempba::load_balancer *p_load_balancer) :
        m_node_manager(p_node_manager), m_load_balancer(p_load_balancer) {
        m_explore_func = std::bind(&multithreading_benchmark::explore, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5);
    }
...
public:
    ...
    void explore(std::thread::id p_tid, const int p_depth, int p_max_depth, std::vector<double> p_arg, gempba::node p_parent) {
        if (p_depth == p_max_depth) {
            ...
            std::scoped_lock v_lock(m_output_mutex);
            ++m_leaf_count;
            m_node_manager.try_update_result(m_leaf_count, gempba::score::make(m_leaf_count));
            return;
        }
        ...
        bool v_left_submitted = m_node_manager.try_local_submit(v_left);
...
#endif //GEMPBA_MULTITHREADING_BENCHMARK_HPP
